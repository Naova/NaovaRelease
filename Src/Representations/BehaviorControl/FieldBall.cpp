...
bool FieldBall::teamBallWasSeen(int timeInterval) const
{
    TeamData* theTeamData = nullptr;
    if(Blackboard::getInstance().exists("TeamData"))
    {
      theTeamData = static_cast<TeamData*>(&(Blackboard::getInstance()["TeamData"]));
      const unsigned long teammatesSize = theTeamData->teammates.size();
    
      if (teammatesSize > 0)
      {
        return timeSinceTeamBallWasValid <= timeInterval;
      }
    }
