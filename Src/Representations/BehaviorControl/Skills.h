...
namespace Skills
{
  ...
  /**
   * This skill turns the robot the specified number of rotations.
   * @param angle The number of rads we want to turn. Can be greater than 2pi
   */
  SKILL_INTERFACE(TurnInPlace, (float)(2*pi) angle);

  /**
   * This skill make a wall of one robot to block the goal.
   * @param wallDistance The distance from the ball in the direction of the goal
   */
  SKILL_INTERFACE(Wall, (float)(950.f) wallDistance);
  ...
  /**
   * This skill uses the GoToBallAndKick skill and kicks with the foot that is closest to the ball
   * @param kickTarget The position of where we want to kick
   */
  SKILL_INTERFACE(SmartKick, (Vector2f) kickTarget);

  /**
   * This skill searches for the ball in a specified area
   * @param searchArea The area we want to search
   */
  SKILL_INTERFACE(SearchBall);
