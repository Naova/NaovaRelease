...
void GlobalFieldCoverage::draw() const
{
  ...
  DEBUG_DRAWING("representation:GlobalFieldCoverage:coverage", "drawingOnField")
  {
    ...
    for(size_t i = 0; i < grid.size(); ++i)
    {
      ...
      RobotPose* theRobotPose = nullptr;
      if(Blackboard::getInstance().exists("RobotPose"))
      {
        theRobotPose = static_cast<RobotPose*>(&(Blackboard::getInstance()["RobotPose"]));

        for(size_t j = 0; j < 4; j++)
        {
          grid2[j] = Vector2f(theRobotPose->inversePose * c.polygon[j]);
        }
      }
