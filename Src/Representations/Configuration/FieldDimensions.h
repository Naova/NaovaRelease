...
public:
  ...
  */
  ...
  /**
  * Returns true when p is inside the own penalty area.
  */
  bool isInOwnPenaltyArea(const Vector2f& p) const
  {
    return p.x() <= xPosOwnPenaltyArea && p.x() >= xPosOwnGroundLine && p.y() <= yPosLeftPenaltyArea && p.y() >= yPosRightPenaltyArea;
  }

  /**
  * Returns true when p is inside the own goal area.
  */
  bool isInOwnGoalArea(const Vector2f& p) const
  {
    return p.x() <= xPosOwnGoalArea && p.x() >= xPosOwnGroundLine && p.y() <= yPosLeftGoalArea && p.y() >= yPosRightGoalArea;
  }
