...
#else
    sheet = "QLabel { background-color : #777777; border: 1px solid silver; }";
#endif
  else if(value >= 500)
...
#else
  QString sheet = "QProgressBar::chunk { background-color: lime; } QProgressBar { background-color : "
                  "#666a70; border: 1px solid silver; }";
#endif
  if(power && (*power)[robot->name].isValid())
  {
    if(charging)
      sheet = "QProgressBar::chunk { background-color: lime; } QProgressBar { color: white; }";
    else
      sheet = "QProgressBar::chunk { background-color: red; } QProgressBar { color: white; }";
  }
