...
BallPercept::Status YoloBallDetector::searchBallOnImage(const CameraImage &image, float* const detected_ball, bool is_upper, Geometry::Line horizon) {
  ...
  if(get_yolo_ball_input_height(is_upper) != image.height || get_yolo_ball_input_width(is_upper) != image.width) {
    CameraImage img(get_yolo_ball_input_width(is_upper), get_yolo_ball_input_height(is_upper));
