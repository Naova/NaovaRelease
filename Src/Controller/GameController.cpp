  ...
  if(automatic & bit(penalizeIllegalPosition))
  {
    for(int i = 0; i < numOfRobots; ++i)
    {
      ...
      if(r.info.penalty == PENALTY_NONE && r.simulatedRobot)
      {
        ...
        const float xMarginOpponentGoalArea = isFirstTeam ? (fieldDimensions.xPosOwnGoalArea + footLength - r.lastPose.translation.x()) : (r.lastPose.translation.x() - fieldDimensions.xPosOpponentGoalArea + footLength);
        ...
        r.opponentPenaltyAreaMargin = std::min(yMarginGoalArea, xMarginOpponentGoalArea);
      }
      else
      {
        r.ownPenaltyAreaMargin = r.opponentPenaltyAreaMargin = -1.f;
        r.ownGoalAreaMargin = r.opponentGoalAreaMargin = -1.f;
      }
    }
  }
  ...
  for(int i = 0; i < numOfRobots; ++i)
  {
    ...
    if(r.info.penalty == PENALTY_NONE && r.simulatedRobot)
    {
      ...
      auto inFullGoalArea = [&]
      {
        ...
        if(r.opponentGoalAreaMargin >= 0.f)
        {
          int counter = 0;
          for(int j = isFirstTeam ? 0 : numOfRobots / 2; j < (isFirstTeam ? numOfRobots / 2 : numOfRobots); ++j)
            if(i != j && (robots[j].opponentGoalAreaMargin > r.opponentGoalAreaMargin || (robots[j].opponentGoalAreaMargin == r.opponentGoalAreaMargin && j < i)))
              ++counter;
          if(counter >= 3)
            return true;
        }
      ...
      if(automatic & bit(penalizeLeavingTheField) && !fieldDimensions.isInsideCarpet(r.lastPose.translation))
        VERIFY(handleRobotCommand(i, "leavingTheField"));
              ...
              (inCenterCircleBeforeBallIsInPlay() || inPenaltyAreaDuringPenaltyKick() || inFullGoalArea() || inFreeKickArea()))
