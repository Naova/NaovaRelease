...
bool GameController::handleGlobalCommand(const std::string& command)
{
  ...
  else if(command == "playing")
  {
    ...
    if((gameInfo.competitionPhase == COMPETITION_PHASE_PLAYOFF && gameInfo.competitionType == COMPETITION_TYPE_NORMAL) || !timeWhenHalfStarted)
      ...
      case STATE_PLAYING:
        switch(updateBall())
        {
          case goalByFirstTeam:
            ++teamInfos[0].score;
            VERIFY(handleGlobalCommand("kickOffSecondTeam"));
            VERIFY(handleGlobalCommand("ready"));
            break;
          case goalBySecondTeam:
            ++teamInfos[1].score;
            VERIFY(handleGlobalCommand("kickOffFirstTeam"));
            VERIFY(handleGlobalCommand("ready"));
            break;
          case outByFirstTeam:
...
void GameController::addCompletion(std::set<std::string>& completion) const
{
  static const char* commands[] =
  {
    ...
    "outByFirstTeam",
    "outBySecondTeam",
    "gameMixedTeamPlayoff",
    "gameMixedTeamRoundRobin",
    "gamePlayoff",
    "gameRoundRobin"
  };
