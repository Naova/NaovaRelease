...
void FilteredCurrentProvider::checkMotorMalfunction(FilteredCurrent& theFilteredCurrent)
{
  ...
  {
    if(theFilteredCurrent.legMotorMalfunction && theFrameInfo.getTimeSince(theFallDownState.timeSinceFallen) > (2000))
    {
      theFilteredCurrent.legMotorMalfunction = false; 
    }
    ...
    for(size_t i = 0; i < currents.size(); i++)
    {
      ...
      if(currents[i].average() == 0 && theJointRequest.stiffnessData.stiffnesses[i] >= stiffness && std::abs(theJointRequest.angles[i] - theJointSensorData.angles[i]) >= jointDif)
        {
          fprintf(stderr, "Incrementing malfunction in joint %zu", i);
          flags[i] += 1;
        }
      else
        ...
        if(theFrameInfo.getTimeSince(soundTimestamp) >= motorMalfunctionTime)
        {
          ...
          else
          {
            ...
            if(!theGyroOffset.bodyDisconnect) 
            {
              ...
              fprintf(stderr, "on a affecte theFilteredCurrent.legMotorMalfunction = true");
            }
          }
