  ...
  if(lastExecution)
  {
    for(unsigned i = 0; i < Arms::numOfArms; ++i)
    {
	  ...
	  else
	  {
		  ...
		  if (armMotionSelection.targetArmMotion[arm] == ArmMotionSelection::keyFrameS &&( theLegMotionSelection.targetMotion == MotionRequest::walk|| theLegMotionSelection.targetMotion == MotionRequest::kick)) {
			  armMotionSelection.armKeyFrameRequest.arms[arm] = theArmMotionRequest.armKeyFrameRequest.arms[arm];
			  armMotionSelection.armKeyFrameRequest.arms[arm].motion = ArmKeyFrameRequest::back;
		  }
      }
