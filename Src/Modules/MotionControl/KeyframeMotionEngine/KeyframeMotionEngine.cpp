...
void KeyframePhase::update()
{
  ...
  {
    doHelpMeStuff();
    fprintf(stderr, "fonction doHelpMeStuff");
  }
...
void KeyframePhase::waitForFallen()
{
  ...
  else if (engine.theFilteredCurrent.legMotorMalfunction && engine.motorMalfunctionBreakUp)
  {
    state = EngineState::helpMeState;
    checkHelpMeState = true;
  }
}
...
{
  if (state == EngineState::helpMeState && !engine.theFilteredCurrent.legMotorMalfunction && checkHelpMeState)
  {
    checkHelpMeState = false;
    state = EngineState::recoverFallen; 
    fprintf(stderr, "On est sorti de doHelpMeStuff\n");
    SystemCall::say("Help me prevented");
    return;
  }
  tooManyTries = true;
  wasHelpMe = true;
