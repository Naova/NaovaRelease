...
{
    float phi = 0.8f;   // change de 0.8 a 1.2 pendant tests.
    float phi1 = 1/ phi;
...
// Equation 14
Angle DynamicController::calcS(Angle epsilonPoint, Angle epsilonLatino)
{
    return epsilonPoint + lambda_1 * epsilonLatino+lambda_2 * sig(alpha1, epsilonLatino) + lambda_3 * sig(alpha2, epsilonLatino);
}

// Calcul de Γ0(t) selon l'équation (18) simplifiée
Angle DynamicController::calcTorqueZero(Angle dQWithError, Angle QWithError, Angle epsilonDot) {
    return -(lambda_1 * epsilonDot + lambda_2 * sig(alpha1, QWithError) + lambda_3 * sig(alpha2, QWithError));
}

// Equation 17
Angle DynamicController::calcGammaEqM(Angle ddQd, Angle torqueZero, Angle H_hat) {
    return M * ddQd - M * torqueZero + H_hat;
}

// Equation 19
Angle DynamicController::calcGammaFtM(Angle s)
{
    return -M*K1*s-M*K2*sig(etat1, s)-M*K3*sig(etat2, s)-M*K4*sign(s);
}

// Equation 20
Angle DynamicController::calcGammaM(Angle gammaEqM, Angle gammaFtM)
{
    return gammaFtM + gammaEqM;
}

// Equation 21
Angle DynamicController::calcThetaCmd(Angle theta, Angle thetaPoint, Angle gammaM)
{
    
    return theta + 1/Kp*(Kd*thetaPoint-gammaM);
}

// Equation 13 (peut ressembler a calcErrorPosition(), changer les valeurs entrees de la fonction)
Angle DynamicController::calcEpsilonTilde(Angle fi, Angle epsilon)
{
    return 0.5f * logf((fi + (float)(epsilon)) / (fi - (float)(epsilon)));
}

// Equation 15
Angle DynamicController :: calcDEpsilonTilde(Angle epsilonTilde)
{
...
// Equation 11 (fonction non utilisee)
Angle DynamicController::calcFi(Angle f0, Angle fInf, double l)
{
...
Angle DynamicController::calcAngleNew(Vector3a desiredAngle0, Vector3a desiredAngle1, Vector2a angle0, Vector2a angle1, Vector2a angle2, Angle* testTorque) {
    ...
    *testTorque = desiredAngle0[2];
    ...
    Angle epsilon_dot = calcDEpsilonTilde(QWithError);                      // rRlated to Equation 14
    Angle s = calcS(epsilon_dot, QWithError);

    // Calculate equivalent control component 
    Angle gamma_eq_m = calcGammaEqM(ddQd, calcTorqueZero(dQWithError, QWithError, epsilon_dot), desiredAngle0[2]);  // Related to Equation 17
