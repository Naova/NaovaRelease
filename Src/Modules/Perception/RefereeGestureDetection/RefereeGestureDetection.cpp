...
void RefereeGestureDetection::update(RefereePercept& theRefereePercept)
{
  DEBUG(DECLARE_DEBUG_RESPONSE("debug data:module:RefereeGestureDetection:checks");
        ...
        DECLARE_DEBUG_RESPONSE("module:RefereeGestureDetection:forceDetection"););
  ...
  DEBUG_RESPONSE_ONCE("module:RefereeGestureDetection:forceDetection")
    forceDetectionIfInFrontOfReferee(theRefereePercept);
}
...
void RefereeGestureDetection::forceDetectionIfInFrontOfReferee(RefereePercept& theRefereePercept) const
{
  const Vector2f refereeRelative = theRobotPose.inversePose * theFieldDimensions.refereeOnField(theGameInfo.leftHandTeam);
  if(std::abs(refereeRelative.angle()) <= 30_deg)
    theRefereePercept.gesture = RefereePercept::initialToReady;
}
